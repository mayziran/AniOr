...
通过 GUI 拖放操作，将动漫视频文件整理到对应季度目录。**支持 TV 动画和剧场版**。
...
- 🔍 **TMDB 搜索**：搜索 TV 动画/剧场版并自动获取季度和集数信息
- 🎬 **剧场版支持**：支持剧场版电影整理，多 CD 自动添加后缀
- 🖱️ **拖放匹配**：支持批量拖放和单集拖放两种模式（TV 动画）
- 🔗 **多种模式**：硬链接（推荐）、剪切、复制
- 📊 **实时高亮**：已匹配文件自动绿色标注
- 📁 **子文件夹**：支持子文件夹折叠/展开
...
|--------|------|------|
| **源目录** | 动漫视频所在的文件夹 | ✅ |
| **目标目录** | 整理后输出的文件夹（TV 动画） | ✅ |
| **剧场版目录** | 剧场版整理输出的文件夹 | 可选 |
...
| **extras 忽略** | 在 extras 文件夹生成.embyignore 文件（仅 TV 动画） | 可选（默认开启） |

### 2. 搜索动漫

在右侧搜索框输入动漫名称，点击 🔍 搜索并选择。

- **TV 动画**：搜索后显示季度列表（S1、S2...）
- **剧场版**：搜索后显示单个剧场版标签页
...
字幕文件会自动跟随视频文件移动到对应目录，并使用相同的重命名规则。

- **TV 动画**：`S01E01 - 原字幕名.srt`
- **剧场版**：`电影名 - 原字幕名.srt` 或 `电影名 - 原字幕名 -cd1.srt`
...
**注意**：此功能仅对 TV 动画的 extras 文件夹生效，剧场版模式不会生成该文件。
...
![主界面 3](docs/screenshot_main_3.png)
